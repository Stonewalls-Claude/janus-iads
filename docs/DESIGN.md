# Janus IADS — design (draft 0.3, 2026-09-23)
...
Where they disagree, the datamine wins and the conflict is listed in the build report. Confirmed
...
MOSCOW, PIOTR, NEUSTRASH, Type_052B/C, Type_054A…). **Mod units** (e.g. `CHAP_PantsirS1`, which
looks like a third-party asset pack) are supported but flagged as mods. Profiles never rely on
them unless the mission uses them.
...
|---|---|---|
| 0 | Unit data generated from the DCS datamine + Olympus databases; battery preset data with sources; project skeleton, build, harness; C-RAM/AI engagement probe mission | Unit list and presets verified; probe results logged |
